...
parser.add_argument('--load_weight', type=str, help='the path of weight')
...
def main():
    ...
    listdir = os.listdir(args.load_weight)
    pt_dict = {}
    epo_lst = []
    for d in listdir:
        if 'n0' in d and 'msimclr' in d:
            epo = int(d.split('_')[-1].split('.')[0][1:])
            epo_lst.append((epo,d))
    i = 1
    while(1):
        done = 1
        for e, d in epo_lst:
    ...
    for epo, file in pt_dict.items():
        ...
        # net.backbone.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), net.backbone.fc)
        print('net')

        net.backbone.load_state_dict(torch.load(args.load_weight + file))
        ...
        temporal_name = TemporalName(model=args.model, train_loader=train_loader, val_loader=val_loader, net=net, optimizer=optimizer, scheduler=scheduler,
                                    device=device, epochs=args.epochs, batch_size=args.batch_size, temperature=args.temperature, iter_print=args.iter_print, n_runs=n, dataset=args.dataset, gpu_idx=args.gpu_idx, beta=args.beta, log_path=log_path, epo=epo)
